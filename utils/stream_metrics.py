...
class StreamSegMetrics(Metrics):
    ...
    def _fast_hist(self, label_true, label_pred):
        """
        This method generates the confusion matrix one element at time
        :param label_true: true label of the element
        :param label_pred: predicted label of the element
        :return: confusion matrix
        """
        mask = (label_true >= 0) & (label_true < self.n_classes)
        # label_true[mask].astype(int) represents the row
        # label_pred[mask] represents the column
        # indexing is given by self.n_classes * row + column
        hist = np.bincount(
    ...
    def update(self, label_trues, label_preds):
        """
        This method generates the confusion matrix given numpy arrays of the ground truth and prediction
        :param label_trues: true label of the element
        :param label_preds: predicted label of the element
        :return: confusion matrix
        """
        for lt, lp in zip(label_trues, label_preds):
            # flatten() is used to get a numpy array and not a single element in order to
            # use mask indexing in the self._fast_hist() function
